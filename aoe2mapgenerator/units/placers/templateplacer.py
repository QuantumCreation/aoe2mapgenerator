...
def _convert_value_to_enum(string: str) -> Union[None, int, float, str]:
    ...
    try:
        ...
        raise ValueError(f"The value \'{string}\' of type \'{type(string)}\'is not a valid enum type.")
        
    try:
        return globals()[enum_type][enum_value]
    except:
        raise ValueError(f"No enum type \'{enum_type}\' with value \'{enum_value}\' was found.")
