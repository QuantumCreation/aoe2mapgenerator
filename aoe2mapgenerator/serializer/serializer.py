    ...
    for map_layer_type_string in serialized_map['layers']:
        ...
        for i in range(len(serialized_map_layer)):
            for j in range(len(serialized_map_layer[i])):
                [aoe2_object, player_id] = _map_element_to_obj_player_id_tuple(serialized_map_layer[i][j])
    ...
    if value == None:
        ...
        try:
            ...
            if type(value[0]) == int:
                ...
                return _convert_enum_instance_to_string(value[0]), _convert_enum_instance_to_string(value[1])
        except:
            # print(value[0], value[1])
