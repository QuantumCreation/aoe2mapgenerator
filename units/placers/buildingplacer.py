...
class PlacerMixin(MapUtilsMixin):
    ...
    def place_all(self, value_type, array_space_type, obj_type = DEFAULT_OBJECT_TYPE, obj_size = 1,  margin = 0, total = 1):
        """
        ...
        for i in range(total):
            if not self._check_and_place(self, value_type, array_space_type=array_space_type, obj_type=obj_type, obj_size=obj_size, margin=margin):
                return
        
        return

    def place_group(self, value_type, array_space_type, obj_type, obj_size, margin = 0, group_size = 1, clumping = 1):
        """
        TODO
        """
        ...
        for (x,y) in sorted(points_list, key = lambda a: ((a[0]-xrand)**2 + (a[1]-yrand)**2 + random.random()*((clumping)**2))):
            ...
            if self._check_placement(self, value_type, points, (x,y), obj_size=obj_size, margin=margin):
                self._place(self, value_type, (x,y), obj_type=obj_type, obj_size=obj_size, margin=margin)
                placed += 1

        return

    def place_groups(self, value_type, array_space_type, obj_type, obj_size, margin = 0, group_size = 1, groups = 1, clumping = 1):
        """
        TODO
        """
        for i in range(groups):
            self.place_group(self, value_type, array_space_type, obj_type, obj_size, margin, group_size, clumping)
    ...
    def add_borders(self, value_type, array_space_type, border_type, border_margin):
        ...
        for point in points:
            if self._is_on_border(self, points, point, border_margin):
                x, y = point
    ...
    def _check_placement(self, value_type, obj_space, point, obj_size, margin, width = -1, height = -1):
        """
    ...
    def _place(self, value_type, point, obj_type, obj_size, margin):
        """
        ...
        for i in range(-margin, eff_size):
            for j in range(-margin, eff_size):
                self.set_point(self,value_type,x+i,y+j,GHOST_OBJECT_DISPLACEMENT)
        
        self.set_point(x+obj_size//2, y+obj_size//2, obj_type)

        return
            
    def _check_and_place(self, value_type, array_space_type, obj_type, obj_size, margin):
        """
        ...
        for (x,y) in sorted(points, key = lambda _: random.random()):
            if self._check_placement(value_type, points, (x,y), obj_size=obj_size, margin=margin):
                self._place(self, value_type, (x,y), obj_type=obj_type, obj_size=obj_size, margin=margin)
                return True
