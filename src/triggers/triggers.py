...
class TriggerObject:
    ...
    def spawn_infinite_waves(self, x11: int, y11: int, x21: int, y21: int, x12: int, y12: int, x22: int, y22: int, target_x: int, target_y: int, object: UnitInfo, player_id: PlayerId, looping: bool = True):
        """
        ...
        for i in range(0, 1+x12-x11):
            for j in range(0, 1+y12-y11):
                ...
                teleport.new_effect.teleport_object(source_player=player_id, location_x=x21+(i%(abs(x22-x21))), location_y=y21+(j%(abs(y22-y21))), area_x1=x11+i, area_y1=y11+j, area_x2=x11+i, area_y2=y11+j)
        teleport.new_effect.activate_trigger(trigger_id=attack_move.ID)

        # Attack move to the target point
        attack_move.new_effect.attack_move(player_id=player_id, location_x=target_x, location_y=target_y, area_x1=x12, area_y1=y12, area_x2=x22, area_y2=y22)
